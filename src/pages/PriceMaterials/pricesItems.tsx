...
export function PircesItems({ price, allPrices, update }: PricesProps) {
    ...
    const {
        register,
        handleSubmit,
    } = useForm<PriceMetalItems>();

    const onUpdate: SubmitHandler<PriceMetalItems> = async data => {
        console.log(data)
        await axios.put<PriceMetalItems>('http://localhost:8080/api/price-metal-item', data)
        update()
    }
    ...
    return (
        <form className="row">
            ...
            <input type="hidden" defaultValue={currentItem?.id ? currentItem.id : 0} {...register("addid")} />
            <div className="p-2">
                {price.thickness}
            </div>
            <div className="p-2">
                {price.table_name}{currentItem?.table_name ? ' | ' + currentItem.table_name : null}
            </div>
            <div className="p-2">
