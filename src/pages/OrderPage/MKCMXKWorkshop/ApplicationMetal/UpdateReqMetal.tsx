...
type UpdateReqMetalProps = {
	order_id: number
	update: () => void
}

export function UpdateReqMetal({ order_id, update }: UpdateReqMetalProps) {
	...
	const onSubmit: SubmitHandler<RequestMetalType> = async data => {
		...
		setIsLoading(true)
		await apiClient
			.post<RequestMetalType>('workshops-data/metal-application-update', data)
			.then(result => {
				openAlert()
				update()
			})
		...
		<>
			<form action='' onSubmit={handleSubmit(onSubmit)}>
				<button type='submit' className='request-metal-btn' disabled={isLoading}>
					{isLoading ? <Spinner animation='border' size='sm' /> : <FaArrowsRotate />} Обновить заказ
				</button>
			</form>
			<Alert className='alert-fixed' show={alertShow} variant='success'>
				Информация актуализарована успешно
			</Alert>
		</>
	)
}
