...
export function PircesItems({ price, update }: PricesProps) {
    const {
        register,
        handleSubmit,
    } = useForm<PricesServiceItem>();

    const onUpdate: SubmitHandler<PricesServiceItem> = async data => {
        console.log(data)
        await axios.put<PricesServiceItem>('http://localhost:8080/api/price-services-item', data)
        update()
    }
    ...
    return (
        <form className="row">
            ...
            <div className={price.cut_cost === null || price.cut_cost == 0 ? 'p-2 d-none' : 'p-2'}>
                <input type="number" defaultValue={price.cut_cost === null ? 0 : price.cut_cost} {...register("cut_cost", { onBlur: handleSubmit(onUpdate) })} className='form-control' />
            </div>
        </form>
    )
}